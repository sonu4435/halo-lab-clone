...
const HomePage = () => {
  ...
  const cursorWidth = 50; // Example width
  const cursorHeight = 50; // Example height
  ...
  const handleMouseMove = e => {
    if (videoRef.current) {
      ...
      cursorControls.start({
        x: offsetX - cursorWidth / 2, // Center follower
        y: offsetY - cursorHeight / 2 // Center follower
      });
    }
  };
  ...
  const handleMouseLeaveVideo = () => {
    ...
    setCursorFollower({ x: -100, y: -100 }); // Hide cursor follower on leave
  };
        ...
        <div className="InnerS h-[90%] w-[90%] relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          ...
          <div className="contentBottom h-[33rem] rounded-3xl w-[90%] absolute left-1/2 -translate-x-1/2 px-5 bottom-10 flex items-center bg-slate-100">
            ...
            <div className="rightParts w-2/3 h-full">
              <div className="VideoContainer w-full h-full mt-5 overflow-hidden relative">
                ...
                <motion.div
                  className="cursorFollower absolute w-20 h-20 rounded-full bg-white"
                  style={{
                    pointerEvents: "none",
                    zIndex: 9999,
